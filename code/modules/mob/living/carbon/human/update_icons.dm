...
/mob/living/carbon/human/update_damage_overlays()
	...
	for(var/obj/item/organ/limb/O in organs)
		if(O.status == ORGAN_ORGANIC && O.state != ORGAN_REMOVED || O.status == ORGAN_ORGANIC && O.body_part == CHEST)
			if(O.brutestate)
...
/mob/living/carbon/human/proc/update_body()
	...
	if(!dna || dna.mutantrace != "skeleton")
		if(HEAD_ORGAN.state != ORGAN_REMOVED)
					...
					if(mutant_type != "normal")
						if(stat == DEAD)
							if(mutant_type == "plant")
								standing		+= image("icon"=human_parts, "icon_state"="[mutant_type]_[affecting.name]_dead_s", "layer"=-BODYPARTS_LAYER)
...
/mob/living/carbon/human/regenerate_icons()
	...
	update_body() //Handles update_body_parts - RR
	update_hair()
	update_mutations()
